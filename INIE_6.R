...
library(mvnfast)
library(invgamma)
#setwd("C:/Study_2023/BNP/Finding_data/INIE/Total_effect")
  ...
  for(rep in 1: 1000)
  {
    ...
    if(new_m_cluster < length(pro))
    {
      if(new_m_cluster < (nrow(xmupars)+1))
      {
        ...
        for(r in 1: q)
        {
          m[r] <- rnorm(1,sum(m_coef * temp_mat), sqrt(m_sig[r]))
        }
      }else{
    ...
    if(new_m_cluster < length(pro))
    {
      if(new_m_cluster < (nrow(xmupars)+1))
      {
        ...
        for(r in 1: q)
        {
          m1[r] <- rnorm(1,sum(m_coef * temp_mat), sqrt(m_sig[r]))
        }
      }else{
        f <- new_m_cluster - nrow(xmupars)
        ff <- unique(xmupars[,1:2])[f,]
    ...
    for(j in 1: length(unique(sord[,1])))
    {
      ...
      for(l in 1: k_j)
      {
        ...
        for(r in 1: q)
        {
          likem0 <- likem0 * dnorm(m[r], sum(c(1,a0l)*m_coef),sqrt(m_sig[r]))
          likem1 <- likem1 * dnorm(m[r], sum(c(1,a1l)*m_coef),sqrt(m_sig[r]))
          likem1_new <- likem1_new * dnorm(m1[r], sum(c(1,a1l)*m_coef),sqrt(m_sig[r]))
        }
